...
const FREQUENCY = { Yearly: 0, Monthly: 1, Weekly: 2, Daily: 3, Anticipated: 4 } as const;
const PAYMENT_TYPE = { MANUAL: 0, STANDINGORDER: 1, DIRECTDEBIT: 2, SUBSCRIPTION: 3 } as const;
const CATEGORY_TYPE = {
...
const PRIORITY_TYPE = { DESIRED: 0, DEPENDANT: 1, ESSENTIAL: 2 } as const;

type FrequencyKey = keyof typeof FREQUENCY;
type PaymentTypeKey = keyof typeof PAYMENT_TYPE;
type CategoryTypeKey = keyof typeof CATEGORY_TYPE;
type PriorityTypeKey = keyof typeof PRIORITY_TYPE;
...
server.tool(
  "get_transactions_till_payday",
  "Get all transactions due between now and the next payday, with totals and spend-per-week",
  { startingBalance: z.number().optional().describe("Optional current account balance in GBP") },
  async ({ startingBalance }) => {
    const qs = startingBalance != null ? `?startingValue=${Math.round(startingBalance)}` : "";
    const dtp = await apiGet<DtpDto>(`/dtp/current${qs}`);
...
server.tool(
  "days_till_payday",
  "Show how many days remain until the next payday and the payday date",
  {},
  async () => {
    const dtp = await apiGet<DtpDto>("/dtp/current");
    ...
    let msg: string;
    if (diff > 0) msg = `${diff} day${diff === 1 ? "" : "s"} until payday (${fmt(dtp.endDate)})`;
    else if (diff === 0) msg = `Payday is today! (${fmt(dtp.endDate)})`;
    else msg = `Payday was ${Math.abs(diff)} day${Math.abs(diff) === 1 ? "" : "s"} ago (${fmt(dtp.endDate)})`;
...
server.tool(
  "list_transactions",
  "List all transactions tracked in MoneyMan",
  { anticipated: z.boolean().optional().describe("If true, return only anticipated (one-off) transactions") },
  async ({ anticipated }) => {
    const qs = anticipated != null ? `?anticipated=${anticipated}` : "";
    const txns = await apiGet<TransactionDto[]>(`/transaction${qs}`);

    if (!txns.length) return text("No transactions found.");

    const lines = txns.map((t) =>
      `[${String(t.id).padStart(3)}] ${(t.name ?? "").padEnd(35)} ${gbp(t.amount ?? 0).padStart(9)}  active:${t.active}`
    );
...
server.tool(
  "create_transaction",
  "Create a new recurring or one-off transaction (bill, standing order, subscription, etc.)",
  {
  ...
  async ({ name, amount, startDate, frequency, paymentType, categoryType, priorityType, bankAccountId }) => {
    ...
    const result = await apiPost<{ id: number }>("/transaction", body);
...
server.tool(
  "update_transaction",
  "Update an existing transaction. Finds by name (fuzzy) if ID not provided.",
  {
  ...
  async ({ id, name, amount, startDate, active, frequency, paymentType, categoryType, priorityType }) => {
    let existing: TransactionDto;

    if (id != null) {
      existing = await apiGet<TransactionDto>(`/transaction/${id}`);
    } else if (name) {
      const all = await apiGet<TransactionDto[]>("/transaction");
      const match = all.find((t) => t.name?.toLowerCase().includes(name.toLowerCase()));
      if (!match) return text(`No transaction found matching "${name}". Use list_transactions to see all.`);
      existing = match;
    } else {
    ...
    const updated: TransactionDto = {
      ...
      ...(startDate && { startDate }),
      ...(active != null && { active }),
      ...(frequency && { frequency: FREQUENCY[frequency as FrequencyKey] }),
      ...(paymentType && { paymentType: PAYMENT_TYPE[paymentType as PaymentTypeKey] }),
      ...(categoryType && { categoryType: CATEGORY_TYPE[categoryType as CategoryTypeKey] }),
      ...(priorityType && { priorityType: PRIORITY_TYPE[priorityType as PriorityTypeKey] }),
    };
    ...
    if (amount != null) changes.push(`amount → ${gbp(amount)}`);
...
server.tool(
  ...
  { id: z.number().int().describe("Transaction ID to delete") },
  async ({ id }) => {
...
server.tool(
  "search_plan_dates",
  "Search scheduled plan dates by transaction name",
  { transactionName: z.string().describe("Transaction name to search for") },
  async ({ transactionName }) => {
    const planDates = await apiGet<PlanDateDto[]>(
      `/plandate/search?transactionName=${encodeURIComponent(transactionName)}`,
    );

    if (!planDates.length) return text(`No plan dates found for "${transactionName}"`);
...
server.tool(
  "mark_plan_date_paid",
  "Mark a specific plan date entry as paid by its ID",
  { id: z.number().int().describe("Plan date ID to mark as paid") },
  async ({ id }) => {
...
server.tool(
  "generate_plan_dates",
  "Regenerate the DTP schedule (plan dates) for all transactions or a single one",
  { transactionId: z.number().int().optional().describe("Limit regeneration to a specific transaction ID") },
  async ({ transactionId }) => {
